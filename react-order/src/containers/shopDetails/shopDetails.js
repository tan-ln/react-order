...
class ShopDetails extends Component {
  ...
  async componentDidMount () {
    ...
    if (typeof restMessage === 'string') {
      ...
      document.title = restMessage
    }
    if (typeof restMessage === 'object') {
      this.setState({
        restMessage
      })
      document.title = restMessage.rst ? restMessage.rst.name : restMessage.restaurant.name
    }
  }
  ...
  render () {
    ...
    const shopContainer = (
      <div>
        <ShopHeader headerInfo={ this.state.restMessage.rst || this.state.restMessage.restaurant } />
        <TabsSmart menu={this.state.restMessage.menu } shopId={ this.props.id } restMessage={ this.state.restMessage.rst || this.state.restMessage.restaurant } />
        {
          this.state.news ? <div className="news" style={{ textAlign: 'center', marginTop: '50%', fontSize: '26px', color: 'red' }}>{ this.state.news }</div> : null
        }
      </div>
      )
